...
void SolarSys::addPlanet(Planet newPlanet){
...
PYBIND11_MODULE(_system, m){
  py::class_<Planet>(m,"Planet")
    .def(py::init<std::string,double,std::vector<double>,std::vector<double>,std::vector<double>>())
  ...
  py::class_<SolarSys>(m, "System")
    ...
    .def("__getitem__",[](SolarSys &sys, size_t index){
          return sys(index);
        });
  py::class_<ODE>(m, "ODE")
    .def(py::init<>())
